...
export const calculateAngle = (x1, y1, x2, y2) => Math.atan2(y2 - y1, x2 - x1);
...
export const formatNumber = (num) => num.toString();
...
export const getAnimationDuration = (distance) => {
...
export const isValidTreeType = (type) => ['BST'].includes(type);
...
export const calculateNodePosition = (
  ...
  const levelWidth = 2 ** level * 100;

  const x = baseX + (index - 2 ** level / 2) * levelWidth;
  const y = baseY + level * levelHeight;

  return { x, y };
};

export const areNodesConnected = (node1, node2, edges) => edges.some(
  (edge) => (edge.from === node1.id && edge.to === node2.id)
