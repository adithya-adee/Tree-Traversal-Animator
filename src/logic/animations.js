import { createAnimationStep } from './utils.js';
...
// use createAnimationStep from utils

export const createStatusUpdate = (
  message,
  duration = ANIMATION_CONFIG.DEFAULT_DURATION,
) => createAnimationStep(ANIMATION_TYPES.UPDATE_STATUS, { message }, duration);

export const createNodeHighlight = (
  nodeId,
  state = HIGHLIGHT_STATES.CURRENT,
  duration = ANIMATION_CONFIG.HIGHLIGHT_DURATION,
) => createAnimationStep(
  ANIMATION_TYPES.HIGHLIGHT_NODE,
  { nodeId, state },
  duration,
);

export const createNodeRecolor = (
  nodeId,
  color,
  duration = ANIMATION_CONFIG.RECOLOR_DURATION,
) => createAnimationStep(
  ANIMATION_TYPES.RECOLOR_NODE,
  { nodeId, color },
  duration,
);

export const createNodeReposition = (
  nodeUpdates,
  duration = ANIMATION_CONFIG.REPOSITION_DURATION,
) => createAnimationStep(ANIMATION_TYPES.REPOSITION, { nodeUpdates }, duration);

export const createEdgeHighlight = (
  edgeId,
  state = HIGHLIGHT_STATES.CURRENT,
  duration = ANIMATION_CONFIG.HIGHLIGHT_DURATION,
) => createAnimationStep(
  ANIMATION_TYPES.HIGHLIGHT_EDGE,
  { edgeId, state },
  duration,
);
...
export const createFadeIn = (
  nodeId,
  duration = ANIMATION_CONFIG.DEFAULT_DURATION,
) => createAnimationStep(ANIMATION_TYPES.FADE_IN, { nodeId }, duration);

export const createFadeOut = (
  nodeId,
  duration = ANIMATION_CONFIG.DEFAULT_DURATION,
) => createAnimationStep(ANIMATION_TYPES.FADE_OUT, { nodeId }, duration);

export const createPulse = (
  nodeId,
  duration = ANIMATION_CONFIG.HIGHLIGHT_DURATION,
) => createAnimationStep(ANIMATION_TYPES.PULSE, { nodeId }, duration);

export const createShake = (
  nodeId,
  duration = ANIMATION_CONFIG.HIGHLIGHT_DURATION,
) => createAnimationStep(ANIMATION_TYPES.SHAKE, { nodeId }, duration);
...
export const generateSearchAnimation = (tree, value) => {
  ...
  while (current) {
    path.push(current);
    animations.push(
      createNodeHighlight(current.id, HIGHLIGHT_STATES.SEARCHING),
    );
...
export const generateInsertionAnimation = (tree, value) => {
  ...
  if (!tree.root) {
    animations.push(
      createStatusUpdate(`Creating root node with value ${value}`),
    );
    return animations;
  }
  ...
  while (current) {
    ...
    if (value < current.value) {
      ...
      animations.push(
        createStatusUpdate(`Value ${value} already exists in the tree`),
      );
      return animations;
    }
  }
...
export const generateDeletionAnimation = (tree, value) => {
  ...
  if (!current.left && !current.right) {
    ...
    animations.push(
      createStatusUpdate(`Deleting node ${value} with one child`),
    );
  } else {
    animations.push(
      createStatusUpdate(`Deleting node ${value} with two children`),
    );
  }
...
export const generateTraversalAnimation = (tree, traversalType) => {
  ...
  animations.push(
    createStatusUpdate(
      `${traversalType} traversal complete: ${result.join(', ')}`,
    ),
  );
...
export const generateAVLRotationAnimation = (pivotNode, rotationType) => {
  ...
  animations.push(
    createStatusUpdate(`AVL Fix: ${rotationType} rotation needed`),
  );
  animations.push(createNodeHighlight(pivotNode.id, HIGHLIGHT_STATES.PIVOT));

  if (pivotNode.left) {
    animations.push(
      createNodeHighlight(pivotNode.left.id, HIGHLIGHT_STATES.PIVOT),
    );
  }
  if (pivotNode.right) {
    animations.push(
      createNodeHighlight(pivotNode.right.id, HIGHLIGHT_STATES.PIVOT),
    );
  }
...
export const generateRedBlackFixAnimation = (caseType, nodes) => {
  ...
  switch (caseType) {
    case 'case1':
      animations.push(
        createStatusUpdate(
          'Fix-up (Case 1): Uncle is Red. Recoloring Parent, Uncle, and Grandparent.',
        ),
      );
      break;
    case 'case2':
      animations.push(
        createStatusUpdate(
          'Fix-up (Case 2): Uncle is Black (Triangle). Performing rotation...',
        ),
      );
      break;
    case 'case3':
      animations.push(
        createStatusUpdate(
          'Fix-up (Case 3): Uncle is Black (Line). Recoloring and rotating...',
        ),
      );
      break;
    default:
...
export const combineAnimations = (...animationSequences) => animationSequences.flat();

export const addDelay = (animations, delay = 500) => animations.map((animation, index) => ({
  ...animation,
  delay: index * delay,
}));
...
export const validateAnimationStep = (step) => {
  ...
  const missingField = requiredFields.find((field) => !(field in step));
  if (missingField) {
